...
object <- cosinor.glmm(Y ~ X + amp_acro(time, group = "X", period = 12), data = vitamind)
...
testdata_two_components_mod$X <- rbinom(length(testdata_two_components$group), 2, prob = 0.5)
object <- cosinor.glmm(Y ~ group + amp_acro(times, n_components = 2, period = c(12,6), group =c("group", "X")), data = testdata_two_components_mod, family = poisson())
...
By default, `xmin` will be set to the minimum time value in the time vector of the original dataframe, and `xmax` will be set to the maximum time value. If we want to focus on a specific region of the plot, we can define use the `xlims` argument to specify the x-bounds.
...
To increase the resolution of the plots, the `pred.length.out` can be increased. If there are multiple periods, the function will automatically generate an appropriate number of points to plot such that the smallest period has sufficient resolution to appreciate cosinor behaviour. This can be adjusted using the `points_per_min_cycle_length` argument which is 20 by default.  

```{r, message=F, warning=F}
object <- cosinor.glmm(Y ~ amp_acro(times, n_components = 2, period = c(12,1000)), data = testdata_period_diff, family = poisson())
...
revolution of the plot $(2\pi)$ represents the period for that
component. The units can be changed to degrees (where $360^\circ$
represents the period), or even to be expressed in the same units as the
period specification and time variable in the data used to fit the
model.
...
clockwise argument, and the location of the `angle = 0` starting point
can be specified with the `start` argument. Hence, if the user wishes to
...
The argument `contour_label_frequency` determines the frequency of contour labels. For example, setting `contour_label_frequency = 2` means that every second contour is labelled. This is useful to avoid cluttered labels when the contour density is high. By default, `contour_label_frequency = 1` which means every contour is labelled.  
