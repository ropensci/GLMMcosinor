...
object <- cosinor.glmm(Y ~ amp_acro(times, period = 12), data = filter(testdata_simple, group == 0), family = poisson())
object
```
...
MESOR (`(Intercept)`). The previous section of this vignette: [An overview of the statistical methods used for parameter estimation](https://rwparsons.github.io/GLMMcosinor/articles/GLMMcosinor.html#an-overview-of-the-statistical-methods-used-for-parameter-estimation)
...
-   `Acrophase estimate = 0.99913`. Note that this estimate is in
    radians to align with conventions. To interpret this, we can express
    `0.99913 radians` as a fraction of the total $2\pi$ and multiply by the period to get the time when the response is a maximal. Hence, $\frac{0.99913}{2\pi} \times 12 = 1.908$ in
    the units of the `time_col` column in the original dataframe. This
    is saying that the peak response would occur after 1.908 time-units and every 12 time-units after this. We can confirm this by plotting:
...
```{r, message=F, warning=F}
object <- cosinor.glmm(Y ~ amp_acro(times, period = 12, group = "group"), data = testdata_simple_gaussian, family = gaussian)
object
```
...
```{r, message=F, warning=F}
object <- cosinor.glmm(Y ~ group + amp_acro(times, period = 12, group = "group"), data = testdata_simple_gaussian, family = gaussian())
object
```
...
```{r, message=F, warning=F}
cosinor.glmm(Y ~ group + amp_acro(time_col = times, n_components = 2, period = c(12, 6), group = c("group", "group")), data = testdata_two_components, family = poisson())
```
...
-   `[group=0]:amp2 = 0.39776` represnts the estimate for amplitude of
    `group 0` for the second component

-   `[group=1]:amp2 = 1.00162` represnts the estimate for amplitude of
...
The `cosinor.glmm()` function allows users to specify formulas for
dispersion and zero-inflation models. These formulas are independent of
the main formula specification:
...
role of the `test_cosinor()` function.
...
also be provided by the `summary.cosinor.glmm()` function, if the
original `cosinor.glmm` object being analysed contains them. The
following demonstration uses the model specified in the **Dispersion and
