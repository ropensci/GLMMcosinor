...
models fit using `{glmmTMB}` (which is what is used by `cglmm()`).

For example, we can apply the functions from `DHARMa` on the `glmmTMB` model
within by accessing it with `$fit`.
