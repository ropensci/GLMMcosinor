...
estimation](https://rwparsons.github.io/GLMMcosinor/articles/GLMMcosinor.html#an-overview-of-the-statistical-methods-used-for-parameter-estimation)
...
DHARMa is an R package used to assess residual diagnostics of regression
models, including the generalised linear models generated by `cglmm()`
[REFERENCE]. This will allow us to evaluate the appropriateness of
...
distribution with `subject` as a random effect variable would be more
appropriate. Firstly, we will compare the plots of both models using
`autoplot()`.
...
mixed_mod_data <- as.data.frame(read_csv("data/mixed_mod_data.csv", 
                                         show_col_types = FALSE))
mixed_mod_data$mouse <-as.factor (mixed_mod_data$mouse) #make 'subject' a factor

#Assuming a Normal distribution 
object_normal_mod <- GLMMcosinor::cglmm(Y ~ group + amp_acro(
  time, 
  1, 
  group = "group", 
  period = 24), 
...
object_gamma_mixed_mod <- suppressWarnings(GLMMcosinor::cglmm(Y ~ group + amp_acro(time, 1, group = "group", period = 24)+ (1 + amp_acro1|mouse), data = mixed_mod_data, family = "Gamma"(link = 'log')))
...
autoplot(object_gamma_mixed_mod, superimpose.data = TRUE, ranef_plot = 'mouse')
```

Based on these figures, it seems that the Gamma model is a better fit.
We will now generate residual plots using the DHARMa package:
