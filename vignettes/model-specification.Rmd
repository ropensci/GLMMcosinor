...
respectively within the group "X".

```{r, message=F, warning=F}
cosinor.glmm(Y ~ 0 + X + amp_acro(time, group = "X", period = 12), data = vitamind)
```
...
```{r, message=F, warning=F}
cosinor.glmm(Y ~ 0 + amp_acro(time, group = "X", period = 12), data = vitamind)
```

Secondly, specification of the amp_acro() function. The amp_acro()
...
-   'group' (the name of the group covariates). Ensure that the names in
    this argument are actually found in the dataset. The group argument
    can be an object or a string.

-   'time' (the name of the time column). Ensure that the name matches
    the name of the time values in the dataset. The 'time' argument can
...
```{r, message=F, warning=F}
testdata <- simulate_cosinor(1000, n_period = 6,
  mesor = 7,
  amp = c(0.1, 0.4, 0.5),
  acro = c(1, 1.5, 0.1),
  beta.mesor = 4.4,
...
```{r, message=F, warning=F}
data(vitamind)
fit <- cosinor.glmm(Y ~ X + amp_acro(X, group = NA, period = 12), data = vitamind)
df_predicted <- predict(fit, newdata = vitamind)
```
