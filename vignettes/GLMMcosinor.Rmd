...
```{r, message=F, warning=F}
data(vitamind)
cosinor.glmm(Y ~ 0 + X + amp_acro(time, 
                                  period = 12, 
                                  group = "X"), 
             data = vitamind)
```
...
```{r}
object <- cosinor.glmm(Y ~ 0 + X + amp_acro(time, 
                                            period = 12, 
                                            group = "X"), 
                       data = vitamind)
...
```{r, message=F, warning=F}
object <- cosinor.glmm(Y ~ group + amp_acro(times, period = 12, group = "group")
...
```{r, message=F, warning=F}
fit <- cosinor.glmm(Y ~ group + amp_acro(times, 
                                         period = 12, 
                                         group = NA), 
