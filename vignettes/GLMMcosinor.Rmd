...
dataset supplied in the `cglmm()` call by creating two
...
presented in as 'Raw Coefficients' in the output of the `cglmm()`
...
The `cglmm()` function is used to fit cosinor models to a variety
...
The most relevant output from the `cglmm()` function is likely to
...
The `cglmm()` function allows you to specify different types of
...
The cglmm() function also allows mixed model specification. See
the
[mixed-models](https://rwparsons.github.io/GLMMcosinor/articles/mixed-models.html)
...
The `predict()` method for `cglmm` objects allows users to get
...
`cglmm()` objects. Firstly, the `autoplot()` method creates a
