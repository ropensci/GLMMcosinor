test_that("model returns accurate parameters", {
  ...
  expect_true((TrueAmp_a * 0.95 < group_a_amp) & (group_a_amp < TrueAmp_a * 1.05))

  group_a_acr <- object$coefficients["group0:acr"]
  expect_true((abs(TrueAcr_a * 0.95) < abs(group_a_acr)) & (abs(group_a_acr) < abs(TrueAcr_a * 1.05)))

  group_a_mes <- object$coefficients[1]
  expect_true((TrueMesor_a * 0.95 < group_a_mes) & (group_a_mes < TrueMesor_a * 1.05))


  group_b_amp <- object$coefficients["group1:amp"]
  expect_true((TrueAmp_b * 0.95 < group_b_amp) & (group_b_amp < TrueAmp_b * 1.05))

  group_b_acr <- object$coefficients["group1:acr"]
  expect_true((abs(TrueAcr_b * 0.95) < abs(group_b_acr)) & (abs(group_b_acr) < abs(TrueAcr_b * 1.05)))

  group_b_mes <- object$coefficients[2]
  expect_true(((TrueMesor_b - TrueMesor_a) * 1.05 < group_b_mes) & (group_b_mes < (TrueMesor_b - TrueMesor_a) * 0.95))
})
