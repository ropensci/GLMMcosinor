...
autoplot.cglmm <- function(
  ...
  if (!is.null(cov_list)) {
    for (i in names(cov_list)) {
      assertthat::assert_that(
        i %in% colnames(object$newdata) && i %in% object$covariates,
        msg = paste(
    ...
    for (i in object$covariates) {
      if (!(i %in% names(cov_list))) {
        ...
        formatted_reference_level <- ifelse(
          is.character(reference_level),
  ...
  if (!missing(xlims)) {
    ...
    timeax <- seq(
      min(object$newdata[object$time_name]),
  ...
  if (any(!is.na(object$ranef_groups))) {
    ...
    # this separates random effect models from fixed effects models
    if (superimpose.data) {
      original_data <- object$newdata
      original_data_processed <- object$newdata
      original_data_processed$levels <- ""
...
data_processor_plot <- function(x, newdata, x_str, ranef_plot = NULL) {
  ...
  if (any(!is.na(x$ranef_groups))) {
    replicated_dfs_total <- NULL
    data_ranef <- newdata

    unique_counts <- sapply(
      x$newdata[ranef_plot],
      function(col) length(unique(col))
    )
